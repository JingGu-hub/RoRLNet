...
def get_dataset(args, use_tsda=None):
    ...
    if args.label_noise_rate > 0:
        noise_train_target, mask_train_target, clean_inds, noise_inds = flip_label(args=args, dataset=train_dataset, target=train_target, ratio=args.label_noise_rate)

    # load train_loader
    train_loader = load_loader(args, train_dataset, noise_train_target)
    # load test_loader
    test_loader = load_loader(args, test_dataset, test_target, shuffle=False)

    return train_loader, test_loader, train_dataset, train_target, test_dataset, test_target, num_classes, clean_inds, noise_inds
